#!/usr/bin/env python
# coding=utf-8
"""Fine-tune PhoBERT with LoRA/MELoRA on undertheseanlp/UDD-v0.1."""
...
class ModelArguments:
    ...
    dataset_name: str = field(
        default="undertheseanlp/UDD-v0.1",
        metadata={"help": "Hugging Face dataset name."},
    )
    tokens_column_name: str = field(default="tokens", metadata={"help": "Token list column name."})
    label_column_name: str = field(default="upos", metadata={"help": "Token label column name."})
...
def ensure_train_validation_test_splits(raw_datasets: DatasetDict, validation_ratio: float, test_ratio: float, seed: int):
    ...
    split = raw_datasets["train"].train_test_split(test_size=test_ratio, seed=seed)
    train_valid = split["train"].train_test_split(
...
def build_label_list(raw_datasets: DatasetDict, label_column_name: str) -> List[str]:
    labels = set()
    for split in raw_datasets:
        for example_labels in raw_datasets[split][label_column_name]:
            labels.update(str(label) for label in example_labels)
...
def tokenize_and_align_labels(
    ...
    for words, word_labels in zip(examples[tokens_column_name], examples[label_column_name]):
        input_ids = []
        aligned_labels = []
        ...
        for word, label in zip(words, word_labels):
            ...
            aligned_labels.extend([label_id if label_all_tokens else -100] * (len(piece_ids) - 1))
...
def align_predictions_to_words(sample, prediction, aligned_label_ids, label_list, tokens_column_name, label_column_name):
    tokens = []
    pred_index = 0
    correct_count = 0

    for word, ground_truth in zip(sample[tokens_column_name], sample[label_column_name]):
...
def main():
    ...
    raw_datasets = ensure_train_validation_test_splits(
        raw_datasets,
        model_args.validation_ratio,
        model_args.test_ratio,
        training_args.seed,
    )
    required_columns = [model_args.tokens_column_name, model_args.label_column_name]
    missing_columns = [
        column for column in required_columns if column not in raw_datasets["train"].column_names
    ]
    if missing_columns:
    ...
    label_list = build_label_list(raw_datasets, model_args.label_column_name)
    ...
    with training_args.main_process_first(desc="dataset tokenization"):
        tokenized_datasets = raw_datasets.map(
            ...
            desc="Tokenizing and aligning UDD labels",
        )
    ...
    def compute_metrics(predictions_and_labels):
        ...
        for prediction, label in zip(predictions, labels):
            for predicted_id, label_id in zip(prediction, label):
                if label_id == -100:
                    continue
                y_pred.append(int(predicted_id))
        ...
        for label_id in range(len(label_list)):
            true_positive = int(((y_pred == label_id) & (y_true == label_id)).sum())
            false_positive = int(((y_pred == label_id) & (y_true != label_id)).sum())
            false_negative = int(((y_pred != label_id) & (y_true == label_id)).sum())
        ...
        return {
            "accuracy": accuracy,
            "f1": float(np.mean(per_label_f1)) if per_label_f1 else 0.0,
        }
